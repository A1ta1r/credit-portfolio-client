<template>
  <div class="flex-container">
    ...
    <h3>Баланс с {{new Date(startDate).toLocaleDateString('ru', options)}} по {{new Date(endDate).toLocaleDateString('ru', options)}}: <vue-numeric :minus="true" :value="sum" class="form-control-static" :read-only="true"
                                                            decimal-separator="." separator="space" currency="₽" :precision="2"/></h3>
    <table class="table table-bordered">
      <tr :key="event" v-for="event in events">
        <td>{{new Date(event.date).toLocaleDateString('ru', options)}}</td>
        <td>{{event.reason}}</td>
        <td><vue-numeric :minus="true" :value="event.amount" class="form-control-static" :read-only="true"
                         decimal-separator="." separator="space" currency="₽" :precision="2"/></td>
      </tr>
    </table>
  </div>
</template>
      ...
      this.user.paymentPlans.forEach(function (plan) {
        plan.paymentList.forEach(function (payment) {
          if (new Date(end) >= new Date(payment.paymentDate)) {
            let d = new Date(payment.paymentDate).toLocaleDateString('ru', ctx.options)
            ctx.dates.push(d)
            ctx.events.push({
